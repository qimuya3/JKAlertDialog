...
-(void)buildViews{
    ...
    if ( iosVer < 8.0 ) {
        ...
        float dw = rc.size.width;
        float dh = rc.size.height;
        //取得當前Device的方向，來當作判斷敘述。（Device的方向型態為Integer）
        int nO = device.orientation;
        switch ( nO ) {
    ...
    //message
    CGFloat messageHeigh = [self heightWithString:_message fontSize:18 width:AlertWidth-2*AlertPadding];
...
- (void)addButtonItem {
#if 0
    _buttonScrollView = [[UIView alloc]initWithFrame:CGRectMake(0, _alertView.frame.size.height- MenuHeight,AlertWidth, MenuHeight)];
    CGFloat  width;
    if(self.buttonWidth){
    ...
    for ( int idx = 0; idx < _items.count; idx++ ) {
        JKAlertDialogItem * item = [_items objectAtIndex:idx];
        UIButton *button = [UIButton buttonWithType:UIButtonTypeSystem];
        //        button.translatesAutoresizingMaskIntoConstraints = NO;
        button.frame = CGRectMake(idx*width, 1, width, MenuHeight);
        //        //seperator
